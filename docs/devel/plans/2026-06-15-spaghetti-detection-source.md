...
- **`available()` / capability gating**: `available()` returns the source's `capable_` flag. `DetectionManager` resolves capability by probing `printer.objects.list` for the U1-only `defect_detection` module. **Timing is load-bearing:** the probe must run *after* the WebSocket connects — at `DetectionManager::init()` time (during `Application::init_panel_subjects`) the socket is not up yet, so the probe would fail and `capable_` would latch false forever. `init()` therefore only registers a post-connect hook via `MoonrakerClient::add_connected_observer("DetectionManager::refresh_capabilities", …)` (the same mechanism `GcodeErrorRouter` uses for replay-on-reconnect); the actual `refresh_capabilities()` runs on every connect and is idempotent. `U1StockSource::on_print_state()` early-returns when `!capable_`, so on every non-U1 printer the source never fires. (A `PrinterDetector`-identity route was considered but the objects.list probe is the direct capability signal and reuses the existing post-connect hook cleanly.)
