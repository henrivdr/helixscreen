...
TEST_CASE("control view: paused shows enabled Resume (play icon)", "[print_control_view]") {
    auto v = compute_control_button_view(PrintJobState::PAUSED, PendingAction::None,
                                         true, true, true);
...
TEST_CASE("control view: idle disables both buttons", "[print_control_view]") {
    for (auto s : {PrintJobState::STANDBY, PrintJobState::COMPLETE,
...
TEST_CASE("control view: pending Pausing -> hourglass, disabled, transitional label",
          "[print_control_view]") {
    auto v = compute_control_button_view(PrintJobState::PRINTING, PendingAction::Pausing,
                                         true, true, true);
...
TEST_CASE("control view: pending Resuming -> hourglass + Resuming label",
          "[print_control_view]") {
    auto v = compute_control_button_view(PrintJobState::PAUSED, PendingAction::Resuming,
                                         true, true, true);
...
TEST_CASE("control view: missing macro slot disables primary", "[print_control_view]") {
    ...
    auto v3 = compute_control_button_view(PrintJobState::PRINTING, PendingAction::None,
