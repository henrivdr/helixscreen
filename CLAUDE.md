...
**`tok.defer` drops during freeze — startup handshakes need `tok.defer_critical`:** `tok.defer` routes through `queue_update` which silently discards callbacks during `scoped_freeze()`. For ordinary post-startup work this is correct. But the FIRST frame of a Klipper subscription, the first `notify_klippy_ready`, and any "establish baseline state" callback the rest of the app waits on must use `tok.defer_critical(tag, fn)` (routes through `queue_critical`, bypasses freeze, still honors `shut_down_`). Symptom of a missed `defer_critical`: feature works on slow startups, breaks on faster ones; device log shows `[UpdateQueue] DROPPED (frozen): <Backend>::notify_update`. Snapmaker lane-EMPTY bug 2026-05-10 was exactly this. See `LifetimeToken::defer_critical` in `include/async_lifetime_guard.h` and `.claude/skills/helix-threading/SKILL.md`.
