...
void on_restart_confirm(lv_event_t* e) {
    ...
    api->execute_gcode(
        ...
        [api, filename, log_prefix, on_failure]() {
            queue_update("ui_resume_dispatch::restart_start_print",
                         [api, filename, log_prefix, on_failure]() {
                             api->job().start_print(
                                 filename,
                                 [log_prefix, filename]() {
                                     spdlog::info("{} Restart succeeded for {}", log_prefix,
                                                  filename);
                                 },
                                 [log_prefix, on_failure](const MoonrakerError& err) {
                                     spdlog::error("{} start_print after restart failed: {}",
                                                   log_prefix, err.message);
                                     auto user_msg = err.user_message();
                                     queue_update(
                                         "ui_resume_dispatch::restart_start_error",
                                         [user_msg = std::move(user_msg), on_failure]() {
                                             NOTIFY_ERROR(lv_tr("Failed to restart: {}"),
                                                          user_msg);
                                             if (on_failure) on_failure();
                                         });
                                 });
...
void dispatch_prepared_resume(MoonrakerAPI* api,
    ...
    backend->prepare_for_resume(
        slot, [api, dispatch = std::move(dispatch), log_prefix,
